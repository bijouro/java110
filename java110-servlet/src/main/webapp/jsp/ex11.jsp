...
<body>
 ...
 <pre>
   ...
   > JSP 페이지로 서블릿 클래스를 만들 때 _jspService() 매서드 안에 생성되는 객체이다.
   
   > builtin 객체
     >> request : HttpServletReqest      - _jspService()의 파라미터
     ...
     >> page : Object                    - _jspService()의 로컬변수
       - 현재 서블릿을 가리킨다. 즉 this 이다.
     >> exception : Throwable            - _jspService()의 로컬변수
       - JSP 페이지의 isErrorPage 속성 값을 설정하면 이 변수를 사용할 수 있다.
 </pre>
 <hr>
...
// 그러나 다음 "ok"와 같이 임의의 변수는 존재하지 않기 때문에 컴파이 롱류가 발생한다.
 %>
</body>
</html>