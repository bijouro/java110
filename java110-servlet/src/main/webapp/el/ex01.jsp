...
<body>
 ...
 > EL(Expression Language)은 콤마(,)와 대괄호([]) 등을 사용하여 객체의 프로퍼티나,
  List, Set, Map 객체의 값을 쉽게 꺼내고 설정하게 도와주는 문법이다.  
 ...
 > EL에서 사용할 수 있는 객체?
     ...
     >> servletContext : $ { pageContext.servletContext.프로퍼티명 }
         >>> Java Code : pageContext.getServletContext().get프로퍼티명(); 
         
     >> session : $ { pageContext.session.프로퍼티명 }
         >>> 예) $ { pageContext.session.name }
         >>> Java Code : pageContext.getSession().getname();
     
     >> request : $ { pageContext.request.프로퍼티명 }
         >>> Java Code : pageContext.getRequest().get프로퍼티명();
     
 > response
     >> param : $ {param.파라미터명}
         >>> request.getParameger("파라미터명");
     
     >> paramValues : $ { paramValues.파라미터명 }
         >>> request.getParamegerValues("파라미터명"); 
